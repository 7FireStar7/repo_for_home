...
{
	long double TWO_PI = PI * 2.0L; //нужно привести к промежутку [-pi;pi] иначе понадобилось бы слишком много слагаемых в ф-ле тейлора
	x = fmodl(x, TWO_PI);
	if (x > PI)
	{
		x = x - TWO_PI;
	...
	long double result = x;  // Первый член: x
	long double term = x; // Текущий член
	*used_count = 1;

	for (int i = 1; i < N; i++)
	{
		term = -term * x * x / ((2.0L * i) * (2.0L * i + 1.0L)); //используем 2.0L для того чтобы небыло переполнения int при подсчете 
		result = result + term;
		(*used_count)++;
...
long double taylor_cos(long double x, int N, long double calculation_accuracy, int* used_count)
{
	...
	long double result = 1.0L;  // Первый член: 1
	long double term = 1.0L; // Текущий член
	*used_count = 1;
